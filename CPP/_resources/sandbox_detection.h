...
namespace sandboxDetection {
	...
	/// <summary>
	/// Require that a certain domain name (A record) be resolvable. Great for intranet domain names to prevent cloud sandboxing. Calls exit(0) if check fails.
	/// </summary>
	...
	/// <summary>
	/// Require that a certain registry key exists. Calls exit(0) if check fails.
	/// </summary>
	...
	/// <summary>
	/// Require that common MS office applications exist (Word, Powerpoint, Excel) are installed. Calls exit(0) if check fails.
	/// </summary>
	void requireCommonMsOfficeApplicationsInstalled();

	/// <summary>
	/// Require that MS outlook is installed and configured. Calls exit(0) if check fails.
	/// </summary>
	...
	/// <summary>
	/// Require that the filename is not an obvious value like sample.exe, malware.exe. Optionally specify a string to require that the executable name matches. Calls exit(0) if the check fails.
	/// </summary>
	/// <param name="exeName">Optional executable name to match.</param>
	void requireStdExeNameCheck(char* exeName);

	/// <summary>
	/// Require that the hard disk is of a reasonable size. Calls exit(0) if check fails. Calls exit(0) if check fails.
	/// </summary>
	...
	/// <summary>
	/// Require that the system is not booting from a VHD. Calls exit(0) if the check fails. 
	/// </summary>
	...
	/// <summary>
	/// Require that the CPU is not manufactured by manufacturer returned by VMs - Hyper-V, Vmware, Xen, Parallels, Virtualbox. Calls exit(0) if the check fails.
	/// </summary>
